...
class PlayerEspn: 
    def __init__(self, filename, gamedata, dbobj, lgobj):
        ...
        self.dbobj = dbobj
        self.lgobj = lgobj
        self.espn_player_url = 'college-basketball'
    ...
    def _parse(self):
        ...
        for row in self.data:
            team_id = row[0]
            player_name = row[1]
            espn_player_id = row[2]

            insert_data = {
                'game_id': self.gamedata['id'],
                'espn_player_id': espn_player_id,
                'team_id': team_id,
                'url': self.espn_player_url,
    ...
    def insert(self):
        ...
        for player in players:
            ...
            existing_player = self.dbobj.query_dict("""
                ...
                WHERE
                    espn_player_id = %s
                    AND espn_player_url = '%s'
            """ % (player['espn_player_id'], player['url']))
            if existing_player:
                ...
                """ % (player['espn_player_id'], self.espn_player_url))
                if prior:
                    if len(prior) == 1:
                        data = {
                    ...
                    insert_data = {
                        'espn_player_id': player['espn_player_id'],
                        'espn_player_url': self.espn_player_url,
                    ...
                    data = self.dbobj.query_dict("SELECT id FROM player WHERE espn_player_id = %s AND espn_player_url = '%s'" % (player['espn_player_id'], self.espn_player_url))
                    update_data = {
                        'player_id': data[0]['id'],
                        'espn_player_id': player['espn_player_id'],
                        'url': self.espn_player_url,
    ...
    def append(self):
        ...
        for row in self.data:
            ...
            if player_id > 0:
                append_data = {'game_id': self.gamedata['id'], 'espn_player_id': espn_player_id, 'player_id': player_id}
...
def resolveNewPlayers(game, filename, dbobj, lgobj):
    obj = PlayerEspn(filename, game, dbobj, lgobj)
    obj.insert()
